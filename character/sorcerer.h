...
namespace wumpus_game {
    ...
    public:
        typedef bool(Sorcerer::* member_function_ptr_type)(const std::vector<std::string> &user_input_argument);
        ...
        Sorcerer(const std::string name,const std::weak_ptr<BaseTile> init_pos);


        virtual void PerformAction() override;

        bool Cast_spell(const std::vector<std::string> &input_cmds);
    };
}
